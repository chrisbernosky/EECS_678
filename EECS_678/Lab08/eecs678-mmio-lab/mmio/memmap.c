...
int main (int argc, char *argv[])
{
  ...
  int size = fstat(fdin, &statbuf);
    if(size == -1)
    {
      printf("Cannot find the size of the input file.  \n");
    }
  ...
  int find = lseek(fdout, statbuf.st_size - 1, SEEK_SET);
    if(find == -1)
    {
      printf("The location of the last byte could not be found.  \n");
    }
   ...
   */
    ...
    if(write(fdout, " ", 1) == -1)
    {
      printf("The dummy byte could not be written to the last location.\n");
    }
  ...
  src = mmap(NULL, statbuf.st_size, PROT_READ, MAP_SHARED, fdin, 0);
    if(src < 0 )
    {
      printf("Could not mmap to the input file.\n");
    }
  ...
  dst = mmap(NULL, statbuf.st_size, (PROT_READ | PROT_WRITE), MAP_SHARED, fdout, 0);
    if(dst < 0)
    {
      printf("Could not map to the output file. \n");
    }
